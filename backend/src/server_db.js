...
async function ensurePerformanceIndexes() {
  ...
  try {
    await client.query('CREATE INDEX IF NOT EXISTS idx_video_watches_student ON student_video_watches(student_id)');
    ...
    console.log('Performance indexes skip (tables might not exist yet)');
  } finally {
    client.release();
  }
}
...
let monthsObjectiveTableReady = false;
...
async function loadSessionMap(client, allowedMonths = null) {
  let query = 'SELECT id, month_code, session_code, title, video_metadata FROM sessions';
  let params = [];
  if (allowedMonths && allowedMonths.length > 0) {
    query += ' WHERE month_code = ANY($1)';
...
function buildSessionVideoRecord(row) {
  ...
  return {
    ...
    details: metadata.details || { concepts_explained: '', skills_earned: '' },
  };
}
...
async function loadVideoEnabledSessionRows(client, allowedMonths = null) {
  let query = 'SELECT id, month_code, session_code, title, video_metadata FROM sessions WHERE video_metadata IS NOT NULL';
  let params = [];
  if (allowedMonths && allowedMonths.length > 0) {
    query += ' AND month_code = ANY($1)';
    params = [allowedMonths];
  }
  const result = await client.query(query, params);
...
async function loadPdfSessionRows(client, allowedMonths = null) {
  let result;
  try {
    let query = 'SELECT id, month_code, session_code, title, google_drive_url FROM pdf_sessions';
    let params = [];
    if (allowedMonths && allowedMonths.length > 0) {
      query += ' WHERE month_code = ANY($1)';
      params = [allowedMonths];
...
function normalizePendingEncryptedVideoRecord(value) {
  ...
  const conceptsExplained = String(details.concepts_explained || details.conceptsExplained || '').trim();
  const skillsEarned = String(details.skills_earned || details.skillsEarned || '').trim();
  const monthObjective = String(raw.monthObjective || '').trim();
...
async function ensureMonthsObjectiveTable(client) {
  if (monthsObjectiveTableReady) return;
  await client.query(`
    CREATE TABLE IF NOT EXISTS months_objective (
      month_code TEXT PRIMARY KEY,
      objective TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);
  monthsObjectiveTableReady = true;
}

async function loadMonthsObjectives(client, months) {
  await ensureMonthsObjectiveTable(client);
  if (!months || months.length === 0) return new Map();
  const result = await client.query(
    'SELECT month_code, objective FROM months_objective WHERE month_code = ANY($1)',
    [months],
  );
  return new Map(result.rows.map((r) => [normalizeMonthCode(r.month_code), r.objective]));
}

async function buildAuthorizedMonthsPayload(client, allowedMonths) {
  const objectiveMap = await loadMonthsObjectives(client, allowedMonths);
  ...
  const months = allowedMonths.map((month) => {
    ...
    return {
      month,
      objective: objectiveMap.get(month) || '',
      videos,
      pdfs,
      quizzes,
    };
...
async function buildStudentDashboardPayload(client, authState, monthsPayload = null) {
  ...
  return {
    ...
    completedVideoIds: watchedVideos.map((v) => String(v.video_id || '')),
    completedQuizSessionIds: solvedQuizRows.map((r) => String(r.session_id || '')),
  };
...
async function buildAdminStudentsStatisticsPayload(client) {
  ...
  const totalResult = await client.query(
    `SELECT COUNT(*)::int AS count 
     FROM "${studentTable}" 
     WHERE active = TRUE AND COALESCE(TRIM(full_name), '') <> ''`,
  );
  const totalEnrolledStudents = totalResult.rows[0].count;

  // 2. Compliant Students (Aggregated month check)
  // This is slightly complex in SQL due to space-separated months, but we can do it
  const requiredMonths = [];
  for (let m = 1; m <= latestAvailableMonthNumber; m++) requiredMonths.push(`M${m}`);

  const studentsResult = await client.query(
    `SELECT allowed_months FROM "${studentTable}" WHERE active = TRUE AND COALESCE(TRIM(full_name), '') <> ''`,
  );
  const compliantStudentsToCurrentMonth = studentsResult.rows.filter((s) => {
    const allowed = normalizeAllowedMonths(s.allowed_months);
    return requiredMonths.every((rm) => allowed.includes(rm));
  }).length;

  // 3. Most Active Students (SQL Aggregated)
  const mostActiveResult = await client.query(
    `SELECT 
        s.serial_no, 
        s.full_name,
        COALESCE(w.watched_count, 0) as watched_sessions,
        COALESCE(q.solved_count, 0) as solved_quizzes,
        (COALESCE(w.watched_count, 0) + COALESCE(q.solved_count, 0)) as score
     FROM "${studentTable}" s
     LEFT JOIN (
        SELECT student_id, COUNT(DISTINCT video_id) as watched_count 
        FROM student_video_watches GROUP BY student_id
     ) w ON s.id = w.student_id
     LEFT JOIN (
        SELECT student_id, COUNT(DISTINCT session_id) as solved_count 
        FROM quiz_results GROUP BY student_id
     ) q ON s.id = q.student_id
     WHERE s.active = TRUE AND COALESCE(TRIM(s.full_name), '') <> ''
     ORDER BY score DESC, watched_sessions DESC, solved_quizzes DESC, s.full_name ASC
     LIMIT 10`,
  );

  const mostActiveStudents = mostActiveResult.rows.map(r => ({
    serial: String(r.serial_no || '').toUpperCase(),
    fullName: r.full_name,
    watchedSessionsLastMonth: Number(r.watched_sessions),
    solvedQuizzesLastMonth: Number(r.solved_quizzes),
    activityScoreLastMonth: Number(r.score)
  }));

  // 4. Top Score Students
  // We'll still need some JS processing for the percentage calculation if total points aren't in results,
  // but we only fetch what we need.
  const topScoreResult = await client.query(
    `SELECT 
        ...
        qr.score,
        qr.total_questions,
        qs.encrypted_metadata,
        qr.session_id::text as session_id
     FROM quiz_results qr
     JOIN "${studentTable}" s ON s.id = qr.student_id
     LEFT JOIN quiz_sessions qs ON qs.session_id = qr.session_id
     WHERE s.active = TRUE AND COALESCE(TRIM(s.full_name), '') <> ''`,
  );

  const scoreAccumulator = new Map();
  const sessionPointsCache = new Map();

  for (const row of topScoreResult.rows) {
    const studentId = row.serial_no;
    let totalPoints = sessionPointsCache.get(row.session_id);
    ...
    if (totalPoints === undefined) {
      ...
      if (!totalPoints || totalPoints <= 0) {
        totalPoints = Number(row.total_questions) || 0;
      }
      sessionPointsCache.set(row.session_id, totalPoints);
    }

    if (totalPoints > 0) {
      const percent = Math.max(0, Math.min(100, (Number(row.score) / totalPoints) * 100));
      const curr = scoreAccumulator.get(studentId) || { serial: studentId, fullName: row.full_name, sum: 0, count: 0 };
      curr.sum += percent;
      curr.count += 1;
      scoreAccumulator.set(studentId, curr);
    }
  }

  const topScoreStudents = Array.from(scoreAccumulator.values())
    .map(s => ({
      serial: s.serial,
      fullName: s.fullName,
      quizzesCount: s.count,
      averageScorePercent: s.sum / s.count
    }))
    .sort((a, b) => b.averageScorePercent - a.averageScorePercent || b.quizzesCount - a.quizzesCount)
    .slice(0, 10);
...
app.post('/admin/videos', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (videoRecord.monthObjective) {
      await ensureMonthsObjectiveTable(client);
      await client.query(
        `INSERT INTO months_objective (month_code, objective)
         VALUES ($1, $2)
         ON CONFLICT (month_code) DO UPDATE SET objective = EXCLUDED.objective, updated_at = NOW()`,
        [videoRecord.month, videoRecord.monthObjective],
      );
    }
